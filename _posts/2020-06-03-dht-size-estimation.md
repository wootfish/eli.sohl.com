...
Previous work on estimating the size of Kademlia-based DHTs has focused on "walking" subregions of the address space. The idea is to count the number of nodes found in some arbitrary interval, then to scale that count up to provide a size estimate for the full network.[^distribution]
...
This idea seems simple enough, but it turns out that it doesn't actually work. Even if peer density is uniform, there is still the problem of measuring density accurately. Any walk over a region of the address space is going to miss some peers; this, if not compensated for, will lead to an underestimate of the actual peer density, which in turn will lead to an underestimate of network size.
...
The naive way to combine these estimates would be to take their average. With a little more work we can instead compute a least-squares fit to find a value of $$n$$ which minimizes error terms across all our sampled values $$N_1$$ through $$N_k$$. The formula simplifies to $$n = \frac{k(k+1)(2k+1)}{6 \sum_i i N_i} - 1$$.

Both methods are accurate, but the least-squares method turns out to be somewhat more precise than the naive method.

By increasing our sample size we can get arbitrarily accurate estimates for network size, and since lookups can be computed in parallel we can increase the estimate's accuracy without slowing it down at all.
...
The next step is to combine these measurements into an overall size estimate. There are many reasonable ways to do this. I've illustrated the results of two simple strategies below.

The results of taking an unweighted average across each individual size estimate are shown in red. The results of taking unweighted averages for each individual order statistic and then deriving a size estimate from a least-squares fit is shown in green.
...
I truly have no idea what's going on in the top right here. This is an extreme case which I included for completeness, to show how the model behaves even under extreme conditions which are unlikely to show up in real life. Even in this case, where some bizarre biases are evident, the model still maintains a tight margin of error.

If you're interested in the code that generated these figures, see [this gist](https://gist.github.com/wootfish/c96b2eab9da3e3174a5badcb0d582963).[^hacky]

[^hacky]: Apologies, by the way, for the quality of the gist code. It was written with effectiveness, not clarity, at front of mind.

Both methods of consolidating size estimates can be seen to produce fairly accurate results. The least-squares method is somewhat more precise in all cases.

Averages over small numbers of lookups closely resemble beta distributions, while averages over large numbers of lookups resemble the [normal distribution](https://en.wikipedia.org/wiki/Normal_distribution). This is a straightforward consequence of the [central limit theorem](https://en.wikipedia.org/wiki/Central_limit_theorem).

Just for completeness, here are the 95% confidence intervals for the green curves in the charts above.

| n | 3 lookups | 10 lookups | 1000 lookups |
| --- | --- | --- | --- |
| 17 | 17.52 ± 36.64% | 17.17 ± 20.79% | 17.04 ± 11.27% |
| 1000 | 1047.89 ± 44.58% | 1014.29 ± 23.77% | 1000.20 ± 3.44% |
| 100000 | 178506.65 ± 44.76% | 172630.55 ± 23.71% | 170100.83 ± 2.35% |
...
Attacking the routing overlay requires a large-scale horizontal or hybrid attack; these are easy to detect because they register as a _massive_ increase in network size (likely combined with a decline in the network's usefulness). The best defense is simply to deploy more honest peers on the network.
...
The closest honest peer's expected distance from an arbitrary address is $$D_{1}$$. If a (successful) vertical Sybil attack is underway, we will observe that $$D_{k} > E[D_1]$$. The probability of this happening by random chance is extremely low.[^vert-prob] In fact, it is low enough that the test is still reliable even when our estimate of network size is still noisy.
...
Thus, once we have an up-to-date estimate of network size, we also have an extremely reliable test for whether any given address is the target of a vertical Sybil attack. The cost of this test is one address lookup, meaning we can simply run it on every lookup we perform.[^resilience]
